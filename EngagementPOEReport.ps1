...
$eDiscoveryCase = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/security/cases/ediscoveryCases?`$filter=displayName eq 'DataSecurityCheck-Case'" | select -ExpandProperty value
#$comsearch = Get-ComplianceSearch -resultsize unlimited | Where-Object {$_.CreatedTime -gt $poedate}
$comsearch = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/security/cases/ediscoveryCases/$($eDiscoveryCase.id)/searches" | Select-Object -ExpandProperty value
...
$searchoutput = foreach($s in $comsearch){$lastStats = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/security/cases/ediscoveryCases/$($eDiscoveryCase.id)/searches/$($s.id)/lastEstimateStatisticsOperation"; $s | Select-Object @{N='Name';E={$_.displayName}},@{N='ContentMatchQuery';E={$_.contentQuery}},@{Name='CreatedTime';Expression={$_.createdDateTime.ToString("MMM-dd-yyyy HH:mm:ss")}},@{Name='LastModifiedTime';Expression={$_.lastModifiedDateTime.ToString("MMM-dd-yyyy HH:mm:ss")}},@{Name='JobStartTime';Expression={$lastStats.createdDateTime.ToString("MMM-dd-yyyy HH:mm:ss")}},@{N='CreatedBy';E={$_.createdBy.user.displayName}},@{N='Status';E={([Cultureinfo]'en-GB').TextInfo.ToTitleCase($lastStats.Status)}},dataSourceScopes}
...
if($reporttype -match'POEReport'){
    ...
    $poehtml += ($searchoutput | Where-Object {$_.dataSourceScopes -like "*Mailboxes"} | Select-Object Name,ContentMatchQuery,@{n='ExchangeLocation';e={$_.dataSourceScopes}},CreatedTime,LastModifiedTime,JobStartTime,CreatedBy,Status | Sort-Object -Property CreatedTime) | ConvertTo-Html -PreContent "</p><b>Content Search Results:</b>"
    $poehtml += ($searchoutput | Where-Object {$_.dataSourceScopes -like "*Sites"} | Select-Object Name,ContentMatchQuery,@{n='SharePointLocation';e={$_.dataSourceScopes}},CreatedTime,LastModifiedTime,JobStartTime,CreatedBy,Status | Sort-Object -Property CreatedTime) | ConvertTo-Html -PreContent "<h3>SharePoint Module</h3>$reportstamp <b>Content Search Results:</b>"
