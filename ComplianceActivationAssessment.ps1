...
#project variables
param ($ReportType='Simple',$ReportPath=$env:LOCALAPPDATA,[switch]$LargeTenant=$false,[switch]$UseCustomList=$false,[string]$ListPath,[Switch]$UsePriorLicenseReport=$false,[String]$LicenseReport)
...
If($UsePriorLicenseReport -eq $false){
...
elseif($userpath){
    ...
    Write-Host "Using Existing License File" -ForegroundColor Yellow
}
...
## unless the file is specified during the execution of the script, we are now making the assumption that the LAST License_Report file in the folder is the most recent. 
if($UsePriorLicenseReport){
    
    try{$list = Import-Csv $LicenseReport}
    catch{
        Write-Host "Previous License Report Not listed. Please select the file" -ForegroundColor Yellow
        Add-Type -AssemblyName System.Windows.Forms
        
        $FileBrowser2 = New-Object System.Windows.Forms.OpenFileDialog -Property @{ 
            InitialDirectory = [Environment]::GetFolderPath('LocalApplicationData') 
            Filter = 'CSV (*.csv)|*.csv'
        }
        
        $null = $FileBrowser2.ShowDialog()
        $list = import-csv $FileBrowser2.FileName
    }
    Write-Host $list.userprincipalname | select -First 100 -Unique
}
else{
...
.PARAMETER UsePriorLicenseReport
Prompts to leverage an existing license file generated by the script to populate the report.

.PARAMETER LicenseReport
Provide a direct path to the csv file used for the custom list.  If this paramter is not specificed when using the prior license report, a dialog box will prompt to select a file.
